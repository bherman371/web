      ...
      <div class="post__author author vcard inline-items">
        ...
        <div class="author-date">
          <a class="h6 post__author-name fn">
            You were invited to be the new moderator of <router-link :to="`/s-app/group-page/${request.groupId}`">{{ request.group.name }}</router-link>
          </a>
...
export default {
  name: "SNewModerationInvitation",
  props: ["request"],
  mixins: [userHelper, CurrentUser, groupHelper],
  methods: {
    ...mapActions([
      'acceptModerationInvitation',
      'declineModerationInvitation'
    ]),
    getTimeAgo(time) {
      return moment(time).fromNow();
    },
    accept(){
      this.acceptModerationInvitation(this.request);
    },
    decline(){
      this.declineModerationInvitation(this.request);
    }
  }
};
</script>
