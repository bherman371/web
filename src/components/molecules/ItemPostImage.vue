...
export default {
  ...
  watch: {
    image: function() {
      ...
      this.setImageFile({ imageFile: this.image_file });
  ...
  methods: {
    ...mapActions(["setImage", "setImageFile", "setImageOrientation"]),
    trigger() {
  ...
  mounted() {
    if (this.itemPostFeaturedImage) {
      this.image = this.itemPostFeaturedImage;
      this.image_file = this.imageFile;
    }
  }
};
</script>
